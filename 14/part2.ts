...
function printRobots(robots: Robot[], deltaT: number): void {
  const positions = robots.map(robot => predictPosition(deltaT, robot));
...
let safetyFactor = calculateSafetyFactor(robots, deltaT);
while (safetyFactor > 100_000_000) {
  deltaT += 1;
  safetyFactor = calculateSafetyFactor(robots, deltaT)
}

printRobots(robots, deltaT);
console.log(`Time: ${deltaT}, Safety factor: ${safetyFactor}`);
