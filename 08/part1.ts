  ...
  for (const [_, locations] of antennaLocations) {
    for (let i = 0; i < locations.length; i++) {
      for (let j = i + 1; j < locations.length; j++) {
        ...
        const antiparallelToP1 = subtractVectors(p1, displacement);
        if (inBounds(map, antiparallelToP1)) {
          antinodes.add(serializeVector(antiparallelToP1));
        }

        const antiparallelToP2 = addVectors(p2, displacement);
        if (inBounds(map, antiparallelToP2)) {
          antinodes.add(serializeVector(antiparallelToP2));
        }
      }
    }
  }
